...
def run(
    ...
    source = cfg.DATA + cfg.PARSE_FRAMES,  # file/dir/URL/glob, 0 for webcam
    ...
    min_size = 0, # the minimum size of the bounding box to save crop
):
    source = str(source)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:
                        if abs(xyxy[0] - xyxy[2]) > min_size and abs(xyxy[1] - xyxy[3]) > min_size:
